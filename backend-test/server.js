...
app.get("/error/constraint", (req, res) => {
  res.status(409).json({ error: "Conflict", message: "Unique constraint violation on email field", timestamp: new Date().toISOString(), details: { constraint: "uk_users_email", field: "email", value: "user@example.com", table: "users" } });
});

app.get("/error/deadlock", (req, res) => {
  res.status(500).json({ error: "Internal Server Error", message: "Deadlock detected - Transaction rolled back", timestamp: new Date().toISOString(), details: { error_type: "Deadlock", transaction: "UPDATE users SET balance = balance - 100", waiting_for: "transaction_12345", retries: 3 } });
});

app.get("/error/disk-space", (req, res) => {
  res.status(507).json({ error: "Insufficient Storage", message: "Database disk space exhausted - cannot write data", timestamp: new Date().toISOString(), details: { disk_used: "99.8%", available: "2GB", database: "production_db", growth_rate: "500MB/day" } });
});

app.get("/error/auth-failed", (req, res) => {
  res.status(401).json({ error: "Unauthorized", message: "Database authentication failed - invalid credentials for user 'dbuser'", timestamp: new Date().toISOString(), details: { database: "PostgreSQL", user: "dbuser", host: "db.prod.example.com", port: 5432, attempts: 5 } });
});

app.get("/error/memory", (req, res) => {
  res.status(500).json({ error: "Internal Server Error", message: "Process out of memory - heap size exceeded 2GB limit", timestamp: new Date().toISOString(), details: { heap_used: "2.1GB", heap_total: "2GB", external_memory: "512MB", process_id: 12345, uptime_hours: 48 } });
});

app.get("/error/exception", (req, res) => {
  res.status(500).json({ error: "Internal Server Error", message: "TypeError: Cannot read property 'email' of undefined", timestamp: new Date().toISOString(), details: { file: "controllers/user.js", line: 42, function: "getUserEmail", stack: "at getUserEmail (user.js:42:15)" } });
});

app.get("/error/rate-limit", (req, res) => {
  res.status(429).json({ error: "Too Many Requests", message: "Rate limit exceeded - 1000 requests per minute limit reached", timestamp: new Date().toISOString(), details: { limit: 1000, window: "1 minute", requests_this_window: 1001, retry_after: 45 } });
});

app.get("/error/validation", (req, res) => {
  res.status(400).json({ error: "Bad Request", message: "Validation failed: password must be at least 8 characters", timestamp: new Date().toISOString(), details: { field: "password", rule: "minLength", expected: 8, received: 5, value: "pass" } });
});
...
app.get("/error/cache-miss", (req, res) => {
  res.status(500).json({ error: "Internal Server Error", message: "Redis cache connection failed - falling back to database caused slowdown", timestamp: new Date().toISOString(), details: { service: "Redis Cache", status: "DOWN", fallback: "Direct Database", response_time: "2500ms (normal: 50ms)", request_id: "req_abc123xyz" } });
});
...
app.get("/error/partial", (req, res) => {
  ...
  if (random < 0.3) {
    ...
    return res.status(429).json({ error: "Too Many Requests", message: "Rate limit exceeded", timestamp: new Date().toISOString() });
  }
  res.json({ status: "ok" });
});
